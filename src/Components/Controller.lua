...
function Controller:_startPlaybackButton()
    ...
    playback.Size = UDim2.new(0, 25, 0, 25) -- Adjust the size as needed
...
function Controller:_startScrubber()
    ...
    local function onTouch(actionName, inputState, input)
    ...
    controller.Scrubber.Hitbox.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            ContextActionService:BindAction("DragScrubber", onTouch, false, Enum.UserInputType.Touch, Enum.UserInputType.MouseMovement)
